...
export function LinkInventoryPanel({ scheduleId, candidates }: Props) {
  ...
  return (
    <div className="rounded border bg-white p-4 text-sm space-y-2">
      <h2 className="font-semibold">候補から選択（対象在庫 / 対象個体を紐付ける）</h2>
        ...
        <table className="min-w-full border-collapse border border-black text-xs">
          <thead>
            <tr className="bg-gray-50">
              {['管理番号','メーカー','機種名','在庫数','保管倉庫','個体番号','個体QR','ステータス','備考','一致理由','操作'].map((h) => <th key={h} className="border border-black px-2 py-1 text-left">{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {candidates.map((c) => {
              ...
              return <tr key={key} className={c.isDemoFallback ? "bg-amber-50" : "bg-white"}>
                ...
                <td className="border border-black px-2 py-1"><button className="rounded border border-black px-2 py-1 disabled:opacity-50" onClick={() => onLink(c)} disabled={loadingKey === key || !c.inventoryUnitId}>{loadingKey === key ? "処理中..." : "紐付ける"}</button></td>
              </tr>;
            })}
          </tbody>
        </table>
      </div>
      <p className="text-xs text-gray-600">※ demo fallback候補は薄黄色で表示されます。実データ候補がある場合は表示されません。</p>
    </div>
  );
}
