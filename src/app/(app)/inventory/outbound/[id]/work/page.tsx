...
export default async function OutboundWorkPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-md px-3 py-4 space-y-3"><h1 className="text-xl font-bold">出庫作業</h1>
    <div className="rounded border bg-white p-3 text-sm space-y-1"><p>予定ID: {schedule.id}</p><p>機種: {schedule.modelNameSnapshot}</p><p>予定日: {schedule.expectedDate.toISOString().slice(0,10)}</p><p>販売先: {schedule.buyerName ?? "-"}</p><p>状態: {schedule.status} {schedule.status === "CANCELED" ? "(取消済)" : ""}</p></div>
    <div className="rounded border bg-white p-3 text-sm space-y-1"><p className="font-semibold">対象Unit</p><p>Unit番号: {unit?.displayCode ?? "-"}</p><p>本体番号: {unit?.bodySerialNumber ?? "-"}</p><p>枠番号: {unit?.frameSerialNumber ?? "-"}</p><p>主基板番号: {unit?.mainBoardSerialNumber ?? "-"}</p><p>QR(補助): {unit?.rawQr ?? "-"}</p><p>動確: {unit?.operationCheckStatus ?? "-"}</p><p>ガラス: {unit?.glassStatus ?? "-"}</p><p>釘シート: {unit?.nailSheetStatus ?? "-"}</p><p>検品: {unit?.inspectionStatus ?? "-"}</p><p>メモ: {unit?.memo ?? "-"}</p><p>保管先: {unit?.storageLocationId ?? "-"}</p></div>
    {!unit ? <p className="rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-700">Unit未紐づきです。販売伝票または入庫作業でUnit番号を確認してください。</p> : null}
