...
import { completeOutboundSchedule, getOutboundScheduleSummary, getOutboundSchedules } from "@/features/inventory/server";
...
export default async function OutboundPage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 md:px-6"><InventoryTabs /><h1 className="text-2xl font-bold">発送予定一覧</h1>
    ...
    <div className="mt-4 overflow-x-auto rounded-lg border bg-white"><table className="min-w-[1100px] w-full text-sm"><thead className="bg-slate-50"><tr>{["発送予定ID", "発送予定日", "販売先", "種別", "メーカー", "機種名", "台数", "出庫元", "発送方法", "ステータス", "操作"].map((h) => <th key={h} className="px-3 py-2 text-left">{h}</th>)}</tr></thead><tbody>{schedules.map((s) => { const done = ["SHIPPED", "DELIVERED", "CANCELED"].includes(s.status); const missingLink = !s.inventoryItemId; return <tr key={s.id} className="border-t"><td className="px-3 py-2">{s.id}</td><td className="px-3 py-2">{s.expectedDate.toISOString().slice(0, 10)}</td><td className="px-3 py-2">{s.buyerName ?? "-"}</td><td className="px-3 py-2">{inventoryItemTypeLabel(s.itemType)}</td><td className="px-3 py-2">{s.makerNameSnapshot ?? "-"}</td><td className="px-3 py-2">{s.modelNameSnapshot}</td><td className="px-3 py-2">{formatQuantity(s.quantity)}</td><td className="px-3 py-2">{s.originLocation?.name ?? "-"}</td><td className="px-3 py-2">{shippingMethodLabel(s.shippingMethod)}</td><td className="px-3 py-2"><InventoryStatusBadge status={outboundStatusLabel(s.status)} /></td><td className="px-3 py-2"><div className="flex items-center gap-2">{done ? <span className="text-xs font-medium text-slate-500">発送済</span> : <form action={async () => { "use server"; await completeOutboundSchedule(s.id); redirect("/inventory/outbound"); }}><Button type="submit" disabled={missingLink}>発送完了</Button></form>}{missingLink && !done ? <span className="text-xs text-amber-700">紐付け在庫なし</span> : null}</div></td></tr>; })}</tbody></table></div></div>;
}
