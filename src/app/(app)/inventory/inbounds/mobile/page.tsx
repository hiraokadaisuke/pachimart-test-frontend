...
export default async function Page() {
  ...
  return (
    <div className='space-y-3 p-3'>
      <h1 className='text-lg font-bold'>入庫作業（スマホ）</h1>
      {rows.length === 0 && <div className='rounded border p-3 text-sm text-slate-500'>入庫予定はありません。</div>}
      {rows.map((r) => {
        ...
        return (
          <div key={r.id} className={`rounded border p-3 text-sm shadow-sm space-y-1 ${done ? 'opacity-60' : 'border-blue-300'}`}>
            <div className='text-xs text-slate-500'>{r.expectedDate.toISOString().slice(0, 10)} / 状態: {r.status}</div>
