...
return <main className="mx-auto max-w-xl p-4"><h1 className="text-lg font-bold">棚卸スキャン</h1><form action={action} className="space-y-2"><input autoFocus name="displayCode" className="w-full border p-3" placeholder="displayCode"/><input name="rawQr" className="w-full border p-3" placeholder="rawQr"/><input name="memo" className="w-full border p-3" placeholder="memo"/><button className="w-full bg-slate-900 text-white p-3">照合して保存</button></form>
...
<Link href={`/inventory/stocktakes/${id}`} className="underline">戻る</Link><ul className="mt-3 text-sm">{s.scans.slice(0,10).map((r)=><li key={r.id} className="border-b py-1">{labels[r.matchStatus] ?? r.matchStatus} / {r.scannedDisplayCode ?? r.scannedRawQr}</li>)}</ul></main>}
