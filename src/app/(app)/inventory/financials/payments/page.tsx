...
const ownerUserId=(await cookies()).get("dev_user_id")?.value??DEV_USERS.A.id; const {page,pageSize,skip,take}=parsePagination(searchParams); const where={ownerUserId}; const [rows,totalCount]=await Promise.all([prisma.paymentRecord.findMany({where,orderBy:{createdAt:"desc"},skip,take}),prisma.paymentRecord.count({where})]);
