import type { InventoryItem, InventoryListingStatus, InventoryStatus, MachineModel, Maker, StorageLocation } from "@prisma/client";
...
const listingStatusValueMap: Record<InventoryListingStatus, string> = {
  NOT_LISTED: "未出品",
  LISTED: "出品中",
  NEGOTIATING: "商談中",
  CONTRACTED: "成約済",
...
export function InventoryItemForm({ action, makers, machineModels, storageLocations, item, submitLabel }: {
  ...
  return <form action={action} className="mx-auto max-w-5xl space-y-4 px-4 py-8 md:px-6">
    ...
    <Card><CardHeader><CardTitle className="text-base">保管・ステータス</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm">在庫ステータス<Select name="inventoryStatus" defaultValue={item ? inventoryStatusValueMap[item.inventoryStatus] : "在庫中"}>{inventoryStatuses.map((v) => <option key={v}>{v}</option>)}</Select></label>
