...
type SummaryState = {
  dealCount: SummaryValue;
  uninvoicedCount: SummaryValue;
  unscheduledCount: SummaryValue;
  scheduledCount: SummaryValue;
...
const buildInvoiceDetailHref = (invoiceId: string) =>
  invoiceId.startsWith("S-V-")
    ? `/sales/sales-invoice/vendor/${invoiceId}`
    : invoiceId.startsWith("S-H-")
      ? `/sales/sales-invoice/hall/${invoiceId}`
...
export default function SalesDemoFlowPage() {
  const [summary, setSummary] = useState<SummaryState>({
    dealCount: PACHIMART_DEMO_DEALS.length,
    uninvoicedCount: "-",
    unscheduledCount: "-",
    scheduledCount: "-",
    documentStatus: "対応済",
  ...
  const [latestOutbound, setLatestOutbound] = useState<LatestOutbound | null>(null);
  ...
  useEffect(() => {
    ...
    if (salesInvoiceIds.length === 0) {
      setSummary({ dealCount, uninvoicedCount, unscheduledCount: 0, scheduledCount: 0, documentStatus: "対応済" });
      return;
    }

    void fetch("/api/inventory/outbound/sales-invoice-status", {
      method: "POST",
      ...
      .then(async (response) => {
        ...
        setSummary({
          ...
          uninvoicedCount,
          scheduledCount,
          unscheduledCount: Math.max(salesInvoiceIds.length - scheduledCount, 0),
          documentStatus: latestInvoice ? "対応済" : "デモデータ",
        });
      })
      .catch(() =>
        setSummary({
          dealCount,
          uninvoicedCount,
          unscheduledCount: "取得不可",
          scheduledCount: "取得不可",
          documentStatus: latestInvoice ? "対応済" : "デモデータ",
        }),
      );

    void fetch("/api/inventory/outbound/latest-demo")
      .then(async (response) => {
        if (!response.ok) throw new Error("latest outbound fetch failed");
        const data = (await response.json()) as { latestOutbound?: LatestOutbound | null };
        setLatestOutbound(data.latestOutbound ?? null);
      })
      .catch(() => setLatestOutbound(null));
  }, []);

  const salesInvoiceHref = latestInvoiceId ? buildInvoiceDetailHref(latestInvoiceId) : "/sales/sales-invoice/list";
  const step6Href = latestOutbound ? `/inventory/outbound/${latestOutbound.id}` : "/inventory/outbound";
  ...
  return (
    <div className="space-y-4">
      <div className="border border-slate-300 bg-slate-700 px-4 py-3 text-white">
      ...
      <div className="grid gap-2 md:grid-cols-5">
        {[{ label: "パチマート成約", value: summary.dealCount }, { label: "未伝票化", value: summary.uninvoicedCount }, { label: "出庫予定未作成", value: summary.unscheduledCount }, { label: "出庫予定", value: summary.scheduledCount }, { label: "帳票プレビュー", value: summary.documentStatus }].map((item) => (
          <div key={item.label} className="border border-slate-300 bg-white px-3 py-2">
            ...
            <p className="text-lg font-bold text-slate-800">{typeof item.value === "number" ? `${item.value}件` : item.value}</p>
          </div>
        ))}
      </div>
      <div className="overflow-hidden border border-slate-300 bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-100 text-slate-700">
            <tr>
              <th className="border-b border-slate-300 px-3 py-2 text-left">STEP</th>
              <th className="border-b border-slate-300 px-3 py-2 text-left">内容</th>
              <th className="border-b border-slate-300 px-3 py-2 text-left">状態</th>
              <th className="border-b border-slate-300 px-3 py-2 text-left">操作</th>
            </tr>
          </thead>
          <tbody>
            {steps.map((item) => (
              <tr key={item.step} className="border-b border-slate-200 align-top">
                ...
                <td className="px-3 py-3">
                  <Link href={item.href} className="inline-flex border border-slate-700 bg-slate-700 px-3 py-1 text-xs font-semibold text-white">{item.cta}</Link>
                </td>
              </tr>
            ))}
            <tr className="align-top">
              <td className="px-3 py-3 font-semibold text-slate-700">STEP7</td>
              <td className="px-3 py-3">
                <p className="font-semibold text-slate-900">帳票を確認する</p>
                <p className="text-xs text-slate-600">請求書、売買契約書、発送依頼書、出庫指示書を確認できます。</p>
              </td>
              <td className="px-3 py-3"><span className="border border-slate-300 bg-slate-50 px-2 py-0.5 text-xs text-slate-700">{summary.documentStatus}</span></td>
              <td className="px-3 py-3">
                <div className="flex flex-wrap gap-2">
                  {documentLinks.length > 0 ? (
                    documentLinks.map((doc) => (
                      <Link key={doc.label} href={doc.href} className="inline-flex border border-slate-500 bg-white px-2 py-1 text-xs text-slate-700">{doc.label}</Link>
                    ))
                  ) : (
                    <Link href="/sales/sales-invoice/list" className="inline-flex border border-slate-700 bg-slate-700 px-3 py-1 text-xs font-semibold text-white">販売伝票一覧へ</Link>
                  )}
                </div>
              </td>
            </tr>
