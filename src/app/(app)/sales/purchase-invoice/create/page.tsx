...
export default function PurchaseInvoiceCreatePage() {
  ...
  const handleSelectWithGuard = (record: InventoryRecord) => {
    ...
    if (record.purchaseInvoiceId || linkedInvoice) {
      ...
      if (invoice) {
        ...
        if (shouldOpen) {
          const url = `/sales/purchase-invoice/${invoice.invoiceType}/${invoice.invoiceId}`;
          router.push(url);
        }
      } else {
  ...
  const handleCreateDraft = (type: "vendor" | "hall") => {
    ...
    router.push(`/sales/purchase-invoice/${type === "vendor" ? "vendor" : "hall"}/${draftId}`);
  };
  ...
  const handleSerialInput = (id: string) => {
    ...
    router.push(`/sales/purchase-invoice/serial-input/${id}`);
  };
