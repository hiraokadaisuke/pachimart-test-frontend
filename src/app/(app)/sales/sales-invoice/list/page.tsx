...
export default function SalesInvoiceListPage() {
  ...
  const resolveSelectedInvoiceIds = (targetRows: SalesInvoiceRow[]): string[] => {
    const ids = new Set<string>();
  ...
  const handleDelete = () => {
    ...
    const invoiceIdsToDelete = resolveSelectedInvoiceIds(selectedRows);
  ...
  const handleMerge = () => {
    ...
    const selectedRows = rows.filter((row) => selectedIds.has(row.id));
    const selectedInvoiceIds = resolveSelectedInvoiceIds(selectedRows);
