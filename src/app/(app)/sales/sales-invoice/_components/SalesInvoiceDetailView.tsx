...
export function SalesInvoiceDetailView({ invoiceId, title, expectedType }: Props) {
  ...
  const isPachimartInvoice = (invoice?.remarks ?? "").includes("パチマート");
  const sourceTypeLabel = isPachimartInvoice ? "パチマート" : "既存在庫";
  const pachimartDealId = (invoice?.remarks ?? "").match(/PachimartDealId:\s*([A-Z0-9-]+)/i)?.[1] ?? "-";
          ...
          <table className="w-full border-collapse text-xs">
            <tbody>
              <tr className="bg-amber-50">
                ...
                <th className="border border-gray-300 px-2 py-1 text-left">パチマート取引ID</th><td className="border border-gray-300 px-2 py-1">{isPachimartInvoice ? pachimartDealId : "-"}</td>
