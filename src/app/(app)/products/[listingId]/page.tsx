...
const formatPrice = (exhibit: Exhibit) => {
  if (exhibit.isNegotiable || exhibit.unitPriceExclTax === null) {
    return "応相談";
  }

  return `¥${exhibit.unitPriceExclTax.toLocaleString("ja-JP")}（税抜）`;
};
...
const resolveInquiryStatus = (exhibit: Exhibit, isSellerViewing: boolean) => {
  if (isSellerViewing) {
    return { available: false as const, reason: "出品者は問い合わせを作成できません" };
  }

  if (exhibit.status === "SOLD") {
...
export default async function ProductDetailPage({ params }: { params: { listingId: string } }) {
  ...
  const exhibit = await getPublicListingById(params.listingId);

  if (!exhibit) notFound();

  const isSellerViewing = exhibit.sellerUserId === devUserId;
  const inquiryStatus = resolveInquiryStatus(exhibit, isSellerViewing);
  const seller = await prisma.user.findUnique({
    where: { id: exhibit.sellerUserId },
    select: {
      companyName: true,
      contactName: true,
      tel: true,
  ...
  const removalLabel =
    exhibit.removalStatus === "REMOVED" ? "撤去済" : formatDate(exhibit.removalDate);
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-4 rounded-lg border border-slate-200 bg-white p-5 text-[13px] shadow-sm">
          <div className="space-y-1.5 border-b border-slate-200 pb-3">
            <p className="text-xs font-semibold text-neutral-700">{exhibit.maker ?? "メーカー未設定"}</p>
            <h1 className="text-xl font-bold text-slate-900">{exhibit.machineName ?? "機種名未設定"}</h1>
            <p className="text-xs font-semibold text-emerald-700">在庫{exhibit.quantity}台（{exhibit.allowPartial ? "バラ売り可" : "バラ売り不可"}）</p>
            <p className="text-2xl font-bold text-slate-900">{formatPrice(exhibit)}</p>
          </div>

          <div className="rounded border border-slate-200">
            <SectionHeader>詳細情報</SectionHeader>
            <div className="grid grid-cols-1 gap-2 bg-white p-3 md:grid-cols-2">
              <DetailRow label="種別" value={exhibit.kind} />
              <DetailRow label="枠色" value="-" />
              <DetailRow label="機械送料回数" value={`${exhibit.shippingFeeCount}個口`} />
              <DetailRow label="出庫手数料回数" value={`${exhibit.handlingFeeCount}個口`} />
              <DetailRow label="釘シート" value={exhibit.hasNailSheet ? "あり" : "なし"} />
              <DetailRow label="取扱説明書" value={exhibit.hasManual ? "あり" : "なし"} />
              <DetailRow label="引き取り" value={exhibit.pickupAvailable ? "可" : "不可"} />
        ...
        <div className="space-y-3 text-[13px]">
          ...
          <PurchaseProcedureCard listing={exhibit} inquiryStatus={inquiryStatus} />
        </div>
      </div>
    </MainContainer>
  );
