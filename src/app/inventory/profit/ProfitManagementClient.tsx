...
const getSalesDateKey = (value?: string) => {
  if (!value) return "";
...
export default function ProfitManagementPage() {
  ...
  const filteredPayables = useMemo(() => {
    const filtered = payables.filter((invoice) => {
      ...
      const matchesIssueDate = payableFilters.issueDate
        ? new Date(invoice.createdAt).toISOString().slice(0, 10) === payableFilters.issueDate
        : true;
      const matchesStaff = payableFilters.staff
        ? (invoice.staff ?? "").toLowerCase().includes(payableFilters.staff.toLowerCase())
        : true;
      return matchesId && matchesMaker && matchesModel && matchesSupplier && matchesIssueDate && matchesStaff;
    });
  ...
  const filteredReceivables = useMemo(() => {
    const filtered = receivables.filter((invoice) => {
      ...
      const matchesIssueDate = receivableFilters.issueDate
        ? getSalesDateKey(invoice.issuedDate || invoice.createdAt) === receivableFilters.issueDate
        : true;
      const matchesStaff = receivableFilters.staff
        ? (invoice.staff ?? "").toLowerCase().includes(receivableFilters.staff.toLowerCase())
        : true;
      return matchesId && matchesMaker && matchesModel && matchesCustomer && matchesIssueDate && matchesStaff;
    });
  ...
  const profitRows = useMemo(() => {
    ...
    const filtered = rows.filter((row) => {
      ...
      const issueDate = getSalesDateKey(row.invoice.issuedDate || row.invoice.createdAt);
          ...
          <div className="overflow-x-auto border border-gray-300 bg-white">
            <table className="min-w-full table-fixed border-collapse text-sm">
                  ...
                  <th className="w-[72px] border border-gray-300 px-3 py-3 text-center">
                    <input
                      ...
                      checked={
                        ...
                        filteredPayables.every((invoice) => selectedPayableIds.has(invoice.invoiceId))
                      }
                      onChange={(e) => {
                        const checked = e.target.checked;
                        setSelectedPayableIds((prev) => {
                          const next = new Set(prev);
                          filteredPayables.forEach((invoice) => {
                            if (checked) {
                              next.add(invoice.invoiceId);
                            } else {
              ...
              <tbody>
                {filteredPayables.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="border border-gray-300 px-3 py-6 text-center text-sm text-neutral-600">
                      支払対象の伝票がありません。
                    </td>
                  </tr>
                ) : (
                  filteredPayables.map((invoice) => {
                    ...
                    return (
                      <tr key={invoice.invoiceId} className="hover:bg-slate-50">
                        ...
                        <td className="border border-gray-300 px-3 py-3 text-neutral-800">{maker || "-"}</td>
                        <td className="border border-gray-300 px-3 py-3 text-neutral-800">{modelSummary}</td>
                        <td className="border border-gray-300 px-3 py-3 text-neutral-800">
                        ...
                        <td className="border border-gray-300 px-3 py-3 text-neutral-800">{invoice.staff ?? "-"}</td>
                      </tr>
                    );
                  })
                )}
                  ...
                  <th className="w-[72px] border border-gray-300 px-3 py-3 text-center">
                    <input
                      ...
                      checked={
                        ...
                        filteredReceivables.every((invoice) => selectedReceivableIds.has(invoice.invoiceId))
                      }
                      onChange={(e) => {
                        const checked = e.target.checked;
                        setSelectedReceivableIds((prev) => {
                          const next = new Set(prev);
                          filteredReceivables.forEach((invoice) => {
                            if (checked) {
                              next.add(invoice.invoiceId);
                            } else {
                              next.delete(invoice.invoiceId);
