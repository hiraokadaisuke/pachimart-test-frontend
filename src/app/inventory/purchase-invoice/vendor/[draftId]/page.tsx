  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900">購入伝票登録（業者）</h1>
          <p className="text-sm text-neutral-600">対象在庫に対する業者向けの伝票情報を入力します。</p>
        </div>
        <button
          type="button"
          onClick={handleConfirm}
          className="rounded-md bg-amber-600 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-amber-500"
        >
          確認
        </button>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {items.map((item, index) => (
              <tr key={item.inventoryId} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-3 text-right text-neutral-800">{formatCurrency(item.extra?.remainingDebt as number)}</td>
