...
export type CsvImportIssue = { rowNumber:number; level:"error"|"warning"; message:string };
...
export function parseInventoryImportRows(text: string): { rows: CsvImportRow[]; issues: CsvImportIssue[] } {
  ...
  for (const h of ["種別","機種名","台数"] as HeaderKey[]) if (col[h] < 0) issues.push({ rowNumber: 0, level: "error", message: `必須列がありません: ${h}` });
...
export function validateImportRows(rows: CsvImportRow[]): CsvImportIssue[] {
  ...
  for (const row of rows) {
    if (!row["種別"]) issues.push({ rowNumber: row.rowNumber, level:"error", message:"種別は必須です" });
    if (!row["機種名"]) issues.push({ rowNumber: row.rowNumber, level:"error", message:"機種名は必須です" });
    const qty = Number(row["台数"]);
    if (!row["台数"] || !Number.isInteger(qty) || qty < 1) issues.push({ rowNumber: row.rowNumber, level:"error", message:"台数は1以上の整数で入力してください" });
    if (row["仕入単価"] && Number(row["仕入単価"]) < 0) issues.push({ rowNumber: row.rowNumber, level:"error", message:"仕入単価は0以上です" });
    if (row["販売予定単価"] && Number(row["販売予定単価"]) < 0) issues.push({ rowNumber: row.rowNumber, level:"error", message:"販売予定単価は0以上です" });
    if (row["種別"] && !TYPE_MAP[row["種別"]]) issues.push({ rowNumber: row.rowNumber, level:"error", message:"種別が不正です" });
    if (row["ステータス"] && !STATUS_MAP[row["ステータス"]]) issues.push({ rowNumber: row.rowNumber, level:"error", message:"ステータスが不正です" });
    if (!row["メーカー"]) issues.push({ rowNumber: row.rowNumber, level:"warning", message:"メーカー未設定として取り込みます" });
  }
  return issues;
}
