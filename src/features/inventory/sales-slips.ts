...
export async function listSalesSlips() {
  ...
  return prisma.salesSlip.findMany({ where: { ownerUserId }, include: { lines: true }, orderBy: { createdAt: 'desc' } });
}

export async function getSalesSlip(id: string) {
  const ownerUserId = await resolveCurrentUserId();
  return prisma.salesSlip.findFirst({ where: { id, ownerUserId }, include: { lines: { include: { inventoryUnit: true } }, bankAccount: true } });
}
...
export async function createSalesSlip(formData: FormData) {
  ...
  return prisma.salesSlip.create({
    data: {
      ...
      lines: { create: [{ ownerUserId, machineName, itemType: 'PACHINKO', quantity: 1, salesUnitPrice, purchaseUnitPrice, amount: salesUnitPrice, inventoryUnitId }] },
    },
  });
}
...
export async function listSelectableSalesUnits() {
  const ownerUserId = await resolveCurrentUserId();
  return prisma.inventoryUnit.findMany({ where: { ownerUserId, status: { notIn: ['SHIPPED','CANCELED'] } }, take: 50, orderBy: { updatedAt: 'desc' } });
}
