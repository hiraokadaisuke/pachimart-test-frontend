...
async function clearExistingData() {
  ...
  const devUsers = await prisma.user.findMany({
    where: { OR: [{ devUserId: { in: DEV_USER_IDS } }, { id: { in: DEV_USER_IDS } }] } as any,
    select: { id: true },
  });
  ...
  await (prisma as any).warehouse.deleteMany({ where: { ownerUserId: { in: resolvedUserIds } } });
...
async function seedUsers() {
  ...
  for (const user of USERS) {
    const existing = (await prisma.user.findFirst({
      where: { OR: [{ devUserId: user.id }, { id: user.id }] } as any,
    })) as any;
...
async function seedWarehouses(userIdMap: Map<string, string>) {
  ...
  await (prisma as any).warehouse.createMany({
...
async function seedMastersOnly() {
  ...
  const makerNames = Array.from(new Set(MAKERS.map((maker) => maker.name)));
